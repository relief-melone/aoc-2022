...
impl Folder {
    ...
    pub fn get_size(self) -> i32 {
        ...
        for (_, folder) in self.folders {
            size += Rc::try_unwrap(folder).unwrap().get_size()
        }
        size
    }
    ...
    pub fn tree(&self) -> () {
        ...
        for (key, folder) in self.folders.clone() {
            folder.tree();
        }
    }
    ...
    pub fn get_root(&self) -> Rc<Folder>{
        ...
        let mut root = &mut self.parent_folder.clone().unwrap();

        while let Some(folder) = &root.parent_folder {
    ...
    match words[2]{
        ...
        _ => {
            let folder = match res.folders.get(words[2]){
                Some(f) => {
                    println!("Folder already present. Returning..");
                    f.clone()
                },
                _ => Rc::new(Folder::new(
                    words[2],
...
fn process_dir(words: Vec<&str>, cwd: &Rc<Folder>) -> Rc<Folder> {
    ...
    let mut folders = &mut cwd.folders.clone();
...
fn process_file(words: Vec<&str>, cwd: &Rc<Folder>) -> Rc<Folder> {
    ...
    let mut files = &mut cwd.files.clone();
    files.insert(
        words[1].to_string(),
        File::new(
            words[1],
...
mod test {
    ...
    fn folder_with_subfolder_01(){
        let folder = Folder::new(
            ...
            HashMap::from([
                ("dir1".to_string(), Rc::new(Folder::new(
                    "dir1",
                    HashMap::from([
    ...
    fn process_line_part01_01(){
        ...
        let mut root = Rc::new(Folder::new(
            "/",
            HashMap::new(),
            HashMap::new(),
            None
